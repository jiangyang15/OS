...
asmlinkage long interceptor(struct pt_regs reg) {
	// the system call number passed in eax register
	int sysc = reg.ax;

	if(table[sysc].intercepted) {
		if (table[sysc].monitored == 2 || \
		 (table[sysc].monitored == 1 && \
		  check_pid_monitored(sysc, current->pid))) {
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	if(cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE) {
		...
		if (table[syscall].intercepted == 0 ||
			pid < 0 || (pid > 0 && pid_task(find_vpid(pid), PIDTYPE_PID) == NULL))
			return -EINVAL;

		if (current_uid() != 0) {
			if (pid == 0 || check_pids_same_owner(pid, current->pid) != 0)
				return -EPERM;
		}

		if(cmd == REQUEST_START_MONITORING) {

			if(check_pid_monitored(pid, syscall))
				return -EBUSY;
